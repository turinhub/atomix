...
export default function PixiComponent({ demoType }: PixiComponentProps) {
  ...
  const initBasicDemo = () => {
    ...
    try {
      ...
      // 绘制圆形
      const drawCircle = (ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, color: string, rotation: number) => {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(rotation);
        ctx.beginPath();
      ...
      // 绘制方形
      const drawSquare = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string, rotation: number) => {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(rotation);
        ctx.beginPath();
      ...
      // 绘制三角形
      const drawTriangle = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string, rotation: number) => {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(rotation);
        ctx.beginPath();
      ...
      const animate = () => {
        ...
        shapes.forEach(shape => {
          ...
          // 绘制形状
          if (shape.type === 'circle') {
            drawCircle(ctx, shape.x, shape.y, shape.radius, shape.color, shape.rotation);
          } else if (shape.type === 'square' || shape.type === 'triangle') {
            if (shape.type === 'square') {
              drawSquare(ctx, shape.x, shape.y, shape.size, shape.color, shape.rotation);
            } else {
              drawTriangle(ctx, shape.x, shape.y, shape.size, shape.color, shape.rotation);
            }
          }
        });
        ...
        particles.forEach(particle => {
          ...
          if (mouseX > 0 && mouseY > 0) {
            ...
            if (distance < 100) {
              ...
              // 确保粒子不会超出屏幕
              if (particle.x < particle.size / 2) particle.x = particle.size / 2;
              if (particle.x > canvas.width - particle.size / 2) particle.x = canvas.width - particle.size / 2;
              if (particle.y < particle.size / 2) particle.y = particle.size / 2;
              if (particle.y > canvas.height - particle.size / 2) particle.y = canvas.height - particle.size / 2;
            }
          }
