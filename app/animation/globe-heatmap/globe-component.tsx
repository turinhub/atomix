...
const generatePopulationData = (): DataPoint[] => {
  const cities: DataPoint[] = [
    ...
    { name: "孟买", lat: 19.0760, lng: 72.8777, weight: 0.82 },
    { name: "大阪", lat: 34.6937, lng: 135.5023, weight: 0.58 },
    { name: "纽约", lat: 40.7128, lng: -74.0060, weight: 0.62 },
    { name: "卡拉奇", lat: 24.8607, lng: 67.0011, weight: 0.55 },
    { name: "布宜诺斯艾利斯", lat: -34.6118, lng: -58.3960, weight: 0.48 },
    { name: "重庆", lat: 29.5630, lng: 106.5516, weight: 0.52 },
...
const generateEarthquakeData = (): DataPoint[] => {
  const earthquakeZones = [
    ...
    { region: "地中海-喜马拉雅地震带", centerLat: 35, centerLng: 25, count: 80 },
    ...
    const initGlobe = async () => {
      try {
        ...
        globe
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          .pointRadius((d: any) => d.weight * (demoType === 'population' ? 5 : demoType === 'earthquake' ? 4 : 3))
          .pointResolution(8)
          .pointAltitude(0.03);
    ...
    return (
      <div className="flex items-center justify-center h-[600px] border rounded-lg bg-red-50">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">请确保网络连接正常，并刷新页面重试</p>
        </div>
      </div>
    );
  }
